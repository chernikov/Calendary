...
1. **Кредити** - внутрішня валюта для AI-генерації контенту (фото, моделі)
2. **Календар** - фіксована ціна за друк та доставку готового продукту (в UAH)
...
✅ Незалежність від валютних коливань
...
**Кредит** - внутрішня валюта платформи для оплати AI-генерації.

**Базовий курс:** $1 = 50 кредитів

Це базова одиниця, яка мультиплікує наші реальні витрати на Replicate з маржею.
...
| AI Операція | Модель | Вартість (кредити) | Реальна вартість ($) | Маржа |
|-------------|--------|-------------------|---------------------|-------|
| Fine-tuning моделі | Flux | 145 | 1.159 | 58 крд (150%) |
| Генерація фото | Flux | 14 | 0.1098 | 5.5 крд (154%) |
| Генерація фото | NanoBanana | 3 | 0.02 | 2 крд (200%) |
...
Користувачі купують кредити за гривні з бонусами:
...
Може згенерувати:
- 3 моделі (3 × 145 = 435 кредитів)
- або 39 фото на Flux (39 × 14 = 546 кредитів)
- або 183 фото на NanoBanana (183 × 3 = 549 кредитів)
- або комбінацію
```

### 1.5 Фактична вартість кредиту

```
При купівлі пакету "Starter": 1 кредит = 1 грн
При купівлі пакету "Business": 1 кредит = 0.83 грн (економія 17%)

Середня вартість з урахуванням бонусів: ~0.9 грн/кредит
```
...
- 30 фото × 14 кредитів: 420 кредитів
- Врахування браку (40%): +12 фото × 14 = 168 кредитів
РАЗОМ кредити: 733 кредити

Еквівалент в гривнях (при Standard пакеті ~0.9 грн/крд): ~660 грн

Календар: 999 грн

ЗАГАЛЬНА ЦІНА: ~1,659 грн
```

**Наші витрати:**
```
AI генерація (Replicate):
- Fine-tuning: $1.159 × 41 = 47.5 грн
- 42 фото (з браком 40%): $4.61 × 41 = 189 грн
Разом AI: 236.5 грн

Друк: 510 грн (поштучно) / 355 грн (оптом 20+ шт)
Payment processing (1.5%): 24.9 грн
...
**Дохід від кредитів:**
```
Користувач витратив: 733 кредити
Фактично заплатив: ~660 грн (якщо купив Standard пакет)

Наші витрати на AI: 236.5 грн
Маржа на кредитах: 660 - 236.5 = 423.5 грн (179%)
```

**Маржа на календарі:**
```
Ціна: 999 грн
Собівартість: 510 + 24.9 + 64.9 = 599.8 грн (без AI)
Маржа: 399.2 грн (67%)
```

**Загальна маржа:**
```
Поштучно: 423.5 + 399.2 = 822.7 грн (98%)
Оптом (20+ шт): 423.5 + 554.2 = 977.7 грн (144%)
```
...
РАЗОМ кредити: 271 кредит

Еквівалент в гривнях: ~244 грн

Календар: 999 грн

ЗАГАЛЬНА ЦІНА: ~1,243 грн
```

**Наші витрати:**
```
AI генерація (NanoBanana):
- Fine-tuning: 47.5 грн
...
**Дохід від кредитів:**
```
Користувач витратив: 271 кредит
Фактично заплатив: ~244 грн

Наші витрати на AI: 82 грн
Маржа на кредитах: 244 - 82 = 162 грн (198%)
```

**Загальна маржа:**
```
Поштучно: 162 + 399.2 = 561.2 грн (83%)
Оптом (20+ шт): 162 + 554.2 = 716.2 грн (136%)
```

### 3.2 Порівняльна таблиця сценаріїв

| Сценарій | Кредити | Ціна (грн) | Собівартість | Маржа (грн) | Маржа (%) |
|----------|---------|-----------|-------------|------------|----------|
| Flux (поштучно) | 733 | 1,659 | 836 | 823 | 98% |
| Flux (оптом 20+) | 733 | 1,659 | 681 | 978 | 144% |
| NanoBanana (поштучно) | 271 | 1,243 | 675.6 | 561 | 83% |
| NanoBanana (оптом 20+) | 271 | 1,243 | 520.6 | 716 | 136% |
...
2. Отримує **вітальний бонус 50 кредитів** (можна згенерувати 3 фото на Flux або 16 на NanoBanana)
...
graph TD
    ...
    D -->|Так| E[Запуск fine-tuning: 145 кредитів]
    D -->|Ні| F[Top-up кредитів]
    F --> E
    E --> G[Очікування 90-100 сек]
    G --> H[Модель готова]
    H --> I[Генерація 30 фото по 14 кредитів]
...
- Fine-tuning: 145 кредитів
- 30 фото: 420 кредитів
- Резерв на регенерацію: ~150 кредитів
РАЗОМ: ~715 кредитів

Поточний баланс: 50 кредитів (вітальний бонус)

❌ Недостатньо! Потрібно докупити: 665 кредитів
✅ Рекомендація: Пакет "Premium" (1150 кредитів за 1000 грн)
```

#### Крок 4: Купівля кредитів (Top-up)
```
Користувач переходить на сторінку покупки кредитів
Вибирає пакет "Premium" (1000 грн → 1150 кредитів)
Оплачує через MonoBank
Баланс оновлюється: 50 + 1150 = 1200 кредитів
...
Для кожного місяця:
- Січень: тема "Зима" → 14 кредитів
- Лютий: тема "День Валентина" → 14 кредитів
...
...
Натискає "Регенерувати" → списується 7 кредитів (-50%)
Нове фото генерується → OK ✅

Фінальний баланс: 630 кредитів
```
...
├── AI-генерація: 570 кредитів використано (вже оплачено)
└── Доставка: розраховується при checkout

Ваш баланс кредитів: 630
```
...
```mermaid
graph TD
    A[Кошик] --> B[Введення адреси доставки]
    B --> C[Вибір відділення Нова Пошта]
    C --> D[Розрахунок доставки 70-100 грн]
    D --> E[Підсумок замовлення]
    E --> F[Оплата MonoBank: 999 грн]
    F --> G{Оплата успішна?}
    G -->|Так| H[Замовлення створене]
    G -->|Ні| I[Повторити оплату]
    H --> J[Email підтвердження]
    J --> K[Відправка на друк]
```
...
────────────────────────────────
Календар А3 (настінний)         999 грн
AI-генерація                    ✅ Сплачено кредитами (570)
Доставка (Нова Пошта, Київ)      85 грн
────────────────────────────────
РАЗОМ:                        1,084 грн

Ваш баланс кредитів: 630 (залишок)
...
**Сценарій:**
Користувач має 200 кредитів, запустив fine-tuning (145 кредитів), залишилось 55.
Тепер хоче згенерувати 30 фото × 14 = 420 кредитів, але має лише 55.

**Рішення:**
```
Після fine-tuning:
"Fine-tuning завершено! ✅
Ваш баланс: 55 кредитів

Для генерації 30 фото потрібно 420 кредитів.
❌ Недостатньо кредитів!

Варіанти:
1. Згенерувати частково (3 фото за 42 кредити)
2. Купити кредити зараз (потрібно мінімум 365 кредитів)
3. Використати NanoBanana (30 фото = 90 кредитів ✅)

Рекомендація: NanoBanana швидше та дешевше! 🚀
Або купіть пакет "Standard" (550 кредитів за 500 грн)
```
...
Після генерації залишиться: 55 - 90 = -35

Потрібно докупити: 35 кредитів
Рекомендація: Пакет "Starter" (100 кредитів за 100 грн)
...
Кредити:
- Fine-tuning: 145 кредитів
- 30 фото Flux: 420 кредитів
Разом: 565 кредитів

Вартість кредитів: пакет "Premium" (1150 за 1000 грн)

Календарі:
- 50 шт × 750 грн = 37,500 грн

РАЗОМ: 38,500 грн
На 1 календар: 770 грн
```

**Варіант 2: Індивідуальні дизайни (50 AI-генерацій)**
```
Кредити:
- 50 × 565 кредитів = 28,250 кредитів
- Пакет Business (3,000 грн → 3,600 кредитів) × 8 = 24,000 грн
  (отримують 28,800 кредитів)

Календарі:
- 50 шт × 750 грн = 37,500 грн

РАЗОМ: 61,500 грн
На 1 календар: 1,230 грн
```

**Рекомендація:** Шаблонний дизайн економить 23,000 грн!
...
**Маржа для нас:**
```
Собівартість: 510 + 24.9 + 64.9 = 599.8 грн
Ціна: 999 грн
Маржа: 399.2 грн (67%)
```
...
**Сценарій:**
Користувач витратив 565 кредитів на генерацію, але передумав купувати календар.

**Що відбувається:**
```
Кредити: списані (565 використано)
...
{
    // Розрахунок вартості
    int credits = request.Model == "flux" ? 14 : 3;
          ...
          <div key={pkg.id} className="package-card">
            ...
            {pkg.bonusCredits > 0 && (
              <div className="discount">
                Бонус {Math.round(pkg.bonusCredits / pkg.credits * 100)}%
              </div>
            )}
...
export function GenerateButton({ model, onSuccess }) {
  ...
  const creditsNeeded = model === 'flux' ? 14 : 3;
...
- Average credits balance (середній залишок кредитів)
```
...
```
Сьогодні:
├── Продано кредитів: 15,000 (13,500 грн)
├── Витрачено кредитів: 12,300
├── Залишок у користувачів: 245,000
├── Продано календарів: 18 (17,982 грн)
└── AI витрати: $79 (3,240 грн)

Дохід від кредитів: 13,500 грн
AI витрати: 3,240 грн
Маржа на кредитах: 10,260 грн (76%)

Дохід від календарів: 17,982 грн
Собівартість: 9,180 грн
Маржа на календарях: 8,802 грн (49%)

Загальна маржа: 19,062 грн (61%)
```
...
- Flux: ~715 кредитів (з резервом на регенерацію)
- NanoBanana: ~320 кредитів (з резервом)

**Q: Чи можу я створити календар без AI?**
A: Так! Якщо у вас вже є фото, просто завантажте їх - кредити не потрібні. Заплатите тільки за календар (999 грн).

**Q: Як дізнатися скільки коштує 1 кредит?**
A: При купівлі базового пакету 1 кредит = 1 грн. При купівлі більших пакетів - дешевше (наприклад, Business: 1 кредит = 0.83 грн).
...
✅ **Високу маржу** - 80-140% залежно від сценарію
✅ **Додатковий revenue** - продаж кредитів, PDF, subscriptions
✅ **Незалежність від валюти** - внутрішня валюта не прив'язана до гривні

**Ключові цифри:**
- Курс: $1 = 50 кредитів
- Fine-tuning: 145 кредитів
- Flux фото: 14 кредитів
- NanoBanana фото: 3 кредити
- Маржа на кредитах: 150-200%
- Маржа на календарях: 67-140%
- Середня маржа: 80-140%
