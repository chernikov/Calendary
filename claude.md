...
## 🎯 ВАЖЛИВО: Читати перед початком роботи

**Перед виконанням будь-якої задачі обов'язково прочитай [FRAME.md](./FRAME.md)** — це головний документ, що визначає AI-Driven Architecture підхід для цього проекту.

**FRAME** визначає:
- Роль AI як головного архітектора та інженера
- Роль людини як оркестратора (intent, constraints, validation)
- Цикл розробки: Intent → Frame → AI Architecture → AI Engineering → Review → Release
- Розподіл задач між AI моделями (Claude, GPT, Gemini, Copilot)
- Інваріанти та обмеження проекту
- Структуру документації та рішень
...
### Правила роботи
- **Завжди читайте [FRAME.md](./FRAME.md) перед початком нової задачі**
- Дотримуйтесь AI-Driven Architecture підходу (Intent → Architecture → Engineering)
- Документуйте архітектурні рішення через ADR (Architecture Decision Records)
- Створюйте окрему бренчу для задачі
- Пишіть описові commit messages
- Додавайте коментарі для складної логіки
- Оновлюйте документацію при змінах архітектури
- Тестуйте зміни локально перед commit
- Перевіряйте дотримання INVARIANTS з FRAME.md

### Посилання на ключові документи
- **[FRAME.md](./FRAME.md)** - Головний документ AI-Driven Architecture
- [Architecture](./docs/architecture.md) - Технічна архітектура
- [Code Quality Rules](./docs/code-quality-rules.md) - Стандарти коду
- [QA Testing Plan](./docs/qa-testing-plan.md) - План тестування
